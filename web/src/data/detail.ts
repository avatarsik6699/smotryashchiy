...
function rawPoints(
  ...
  const mine = raw.filter(
    (m) =>
      m.name === name && seriesKey(name, m.labels) === seriesKey(name, labels),
  );
  const points = mine
    .map((m) => ({ t: Date.parse(m.ts), v: m.value }))
    .sort((a, b) => a.t - b.t);
  return withLiveTail(
    points,
    record.series.get(seriesKey(name, labels))?.points,
  );
}
...
function directionRates(
  ...
  for (const m of raw)
    if (m.name === name) interfaces.add(m.labels.interface ?? "");
  for (const s of record.series.values())
    if (s.name === name) interfaces.add(s.labels.interface ?? "");
...
export function diskInfos(record: HostRecord, nowMs: number): DiskInfo[] {
  ...
  for (const s of record.series.values()) {
    if (
      !["disk.used_percent", "disk.used_bytes", "disk.total_bytes"].includes(
        s.name,
      )
    )
      continue;
...
const CONTAINER_FIELDS = {
  "docker.container.cpu_percent": "cpuPercent",
  "docker.container.memory_used_bytes": "memUsedBytes",
  "docker.container.memory_used_percent": "memUsedPercent",
} as const;

/**
 * Current state of every Docker container reporting `docker.container.*` metrics (docs/SPEC.md
 * §4h, §5). `latest=true` has no window, so a redeployed container has one series per image and an
 * exited one keeps its last points: per container and metric the newest series wins, and a
 * container with no fresh sample is not listed. A host with no current containers returns an empty
 * list — the block is omitted entirely rather than shown empty (Docker is optional, unlike disks).
 */
export function containerInfos(
  record: HostRecord,
  nowMs: number,
): ContainerInfo[] {
  const byName = new Map<
    string,
    { info: ContainerInfo; newest: number; fieldTs: Map<string, number> }
  >();
  for (const s of record.series.values()) {
    const field = CONTAINER_FIELDS[s.name as keyof typeof CONTAINER_FIELDS];
    const name = s.labels.container;
    const p = s.points.at(-1);
    if (field === undefined || name === undefined || p === undefined) continue;
    const entry = byName.get(name) ?? {
      ...
      newest: -Infinity,
      fieldTs: new Map<string, number>(),
    };
    if (p.t > (entry.fieldTs.get(field) ?? -Infinity)) {
      entry.fieldTs.set(field, p.t);
      entry.info[field] = isFresh(p.t, nowMs) ? p.v : null;
    }
    if (p.t > entry.newest) {
      entry.newest = p.t;
      entry.info.image = s.labels.image ?? "";
    }
    byName.set(name, entry);
  }
  return [...byName.values()]
    .filter((e) => isFresh(e.newest, nowMs))
    .map((e) => e.info)
    .sort(
      (a, b) =>
...
export function mergeHostEvents(
  fetched: EventDTO[],
  live: EventDTO[],
  hostId: string,
  limit = 20,
